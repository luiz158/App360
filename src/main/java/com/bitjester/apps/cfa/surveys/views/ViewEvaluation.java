...
public class ViewEvaluation implements Serializable {
	...
	private Evaluation initialize(Long id) throws Exception {
		...
		if (0 == e.getAnswers().size()) {
			...
			while (ite.hasNext()) {
				a = new Answer();
				a.setEvaluation(e);
				a.setQuestion(ite.next());
				a.setQindex(a.getQuestion().getQindex());
				e.getAnswers().add(a);
				bk.store(a);
			}
	...
	public List<Behavior> getBehaviorsForCompetence() {
		...
		query += " WHERE survey.id=" + managedEvaluation.getSurvey().getId();
		query += " AND behavior.competence.id=" + competencies.get(index).getId();
		query += ") ORDER BY id";
		return em.createQuery(query, Behavior.class).getResultList();
	}

	public List<Question> questionsForBehavior(Long id) {
		String query = "FROM Question WHERE survey.id=" + managedEvaluation.getSurvey().getId();
	...
	public void setManagedEvaluation(Evaluation managedEvaluation) throws Exception {
